...
uint rgbtouint(float4 c)
{
	c.x = clamp(c.x * 255, 0.0f, 255.0f);
	c.y = clamp(c.y * 255, 0.0f, 255.0f);
	c.z = clamp(c.z * 255, 0.0f, 255.0f);
	return convert_uint(c.x) + (convert_uint(c.y)<<8) + (convert_uint(c.z)<<16);
}

__kernel void raytracer(__global uint *output, uint width, uint height)
{
	...
	output[pos.x + pos.y * width] = rgbtouint(recursivetrace(&s, &r));
}